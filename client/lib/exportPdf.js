...
function buildPDF(taxData, reportData, year) {
  const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
  const W = doc.internal.pageSize.getWidth();
  const H = doc.internal.pageSize.getHeight();
  let y = 0;

  const drawBackground = () => { doc.setFillColor(...DARK); doc.rect(0, 0, W, H, 'F'); };
  const addPage = () => { doc.addPage(); y = 0; drawBackground(); };
  const checkPage = (needed = 20) => { if (y + needed > H - 15) addPage(); };
  ...
  (taxData?.applicableRules || []).forEach((rule) => {
    checkPage(18);
    doc.setFillColor(...CARD);
    ...
    doc.setFontSize(7);
    doc.setFont('helvetica', 'normal');
    doc.text(doc.splitTextToSize(rule.description, W - 46)[0], 32, y + 10);
    y += 18;
  });
...
export function exportTaxReportPDF(taxData, reportData, year) {
  buildPDF(taxData, reportData, year).save(`tax-report-${year}.pdf`);
}

export function previewTaxReportPDF(taxData, reportData, year) {
  return buildPDF(taxData, reportData, year).output('dataurlstring');
}
